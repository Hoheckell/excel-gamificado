<x-guest-layout>
    <x-authentication-card>
        ...
        <div x-data="{ recovery: false }">
            <div class="mb-4 text-sm text-[--text-muted]" x-show="! recovery">
                {{ __('Confirme o acesso à sua conta inserindo o código de autenticação fornecido pelo seu aplicativo autenticador.') }}
            </div>

            <div class="mb-4 text-sm text-[--text-muted]" x-cloak x-show="recovery">
            ...
            <form method="POST" action="{{ route('two-factor.login') }}">
                ...
                <div class="flex items-center justify-end mt-4">
                    <button type="button" class="text-sm text-[--text-muted] hover:text-excel-dark underline cursor-pointer"
                                    x-show="! recovery"
                                    x-on:click="
                                        recovery = true;
